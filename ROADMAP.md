...
- 🟡 Spell effects — *mostly done*: explicit JSON combat fields now win over prose regex; healing/effect-type detection fixed; upcasting via `scaling` + the `at_higher_levels` key (~85 spells); explicit fields for Fireball, Cure Wounds, Lightning Bolt, Chromatic Orb, Magic Missile; `cast_spell` now persists slot consumption to caster_data (route double-deduct removed). **Remaining:** explicit fields for the rest of the regex-resistant damage spells (Disintegrate, Finger of Death, Spiritual Weapon, …); apply effects for the ~36 save-only spells that currently roll a save but apply nothing.
