...
- 🟡 Spell effects — *mostly done* (Codex GPT-5.5 reviewed): explicit JSON combat fields win over prose regex; healing/effect-type detection fixed; upcasting via a shared `_upcast_dice` helper that preserves `NdM+K` flats and differing die sizes, drives BOTH damage and healing, and reads `scaling` + the `at_higher_levels` key (~85 spells); explicit fields for Fireball, Cure Wounds, Lightning Bolt, Chromatic Orb, Magic Missile (incl. correct dart scaling); `cast_spell` persists slot consumption to caster_data (route double-deduct removed).   Disintegrate + Finger of Death enriched (with a `half_damage_on_save` field — Disintegrate deals 0 on save). **Save-only gap investigated:** the resolver/dispatch already apply conditions on a failed save; of the 36 "applies nothing" save spells, the damage ones are now fixed, the **out-of-combat** ones (Plane Shift, Seeming, Imprisonment, Zone of Truth) correctly do nothing in combat, and the **bespoke-effect** ones (Slow, Haste, Bane, Command, Confusion, Polymorph…) need real per-spell implementations — a larger content/engine effort, not a data fix. **Remaining:** bespoke save-spell effects; **selectable damage type for Chromatic Orb** (defaults to fire); Spiritual Weapon (a summoned-weapon attack, also bespoke).
